#%% 
import websocket   # sync - use with Thread, easier to control but not scalable, and not necessarily eaiser to code
...
def subscribe(ws, sub_type, app_key, sub_data):  # 세션 종목코드(실시간체결, 실시간호가) 등록
    ws.send(_build_message(app_key, sub_type, sub_data), websocket.ABNF.OPCODE_TEXT)  # sending text

    time.sleep(.1)


def unsubscribe(ws, sub_type, app_key, sub_data): # 세션 종목코드(실시간체결, 실시간호가) 등록해제
    ws.send(_build_message(app_key, sub_type, sub_data, '2'), websocket.ABNF.OPCODE_TEXT)  # sending text
...
def on_open(ws):
    ...
    subscribe(ws, KIS_WSReq.NOTICE, _connect_key, HTS_ID) # HTS ID 입력


def command_control(ws):
    while True:
        time.sleep(3)  
        print('-----------------------------------------------')
...
# Create a queue to share the WebSocket instance between threads
ws_queue = queue.Queue()

# Start the WebSocket connection in a separate thread
ws_thread = Thread(target=run_ws, args=(ws_queue,))
ws_thread.daemon = True  # Daemon thread will exit when the main program exits
ws_thread.start()

# Get the WebSocket instance from the queue
ws = ws_queue.get()

# Start sending commands periodically
send_thread = Thread(target=command_control, args=(ws,))
