...
- Executing a Module as a Script:  
    ...
    - Wrap your multiprocessing code in an if __name__ == "__main__": block in your main script to ensure that it's only executed in the main process.  
...
- Finding a Package:  
    ...
    **more robust method**  
    ...
    - mac: .zshrc is enough
        - PATH is for executables, while PYTHONPATH is for imports. So, should use PYTHONPATH
...
- in OpenDartReader
    ...
    def finstate(...): 
        ...
        # print('전자공시의 재무데이터는 2015년 이후 데이터를 제공합니다') if bsns_year < 2015 else print() <- make this commented out
        ...
        * may make all 'print' commented out
    ```
...
- fdr: Modify listing.py and data.py to include date_req parameter
    * important: the same applies to KrxMarcapListingCache, which saves data in csv.
    * this allows you to get market data for a specific date; date format '20240101'  
    - [data.py]
...
- dart.py in OpenDartReader
    ...
    - to:
        ...
        def find_corp_code(self, corp):
            ...
            # 1) check stock_code first if length == 6 (case-insensitive)
            if len(corp) == 6:
                df = self.corp_codes[self.corp_codes['stock_code'].str.lower() == corp_lower]

            # 2) if not found, check corp_code (exact match)
            if (df is None or df.empty) and corp in set(self.corp_codes['corp_code']):
                df = self.corp_codes[self.corp_codes['corp_code'] == corp]

            # 3) if still not found, check corp_name (case-insensitive)
            if df is None or df.empty:
                df = self.corp_codes[self.corp_codes['corp_name'].str.lower() == corp_lower]
